...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParIdentite() {
		...
		query.setParameter("identite","%Cotillard%");
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParRole() {
		...
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a JOIN a.roles r WHERE r.nom LIKE :nom", Acteur.class);
	...
	public void testExtraireActeursParPays() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN f.pays p WHERE lower(p.nom) = 'france'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParListePaysEtAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN f.pays p WHERE lower(p.nom) = 'france' AND f.annee = 2017", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a JOIN a.roles r JOIN r.films f JOIN f.realisateurs real WHERE real.identite LIKE :nom", Acteur.class);
		query.setParameter("nom", "%Scott%");
		List<Acteur> acteurs = query.getResultList();
